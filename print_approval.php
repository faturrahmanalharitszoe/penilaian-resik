<?php
include "koneksi.php";
require('pdf/fpdf.php');
date_default_timezone_set("Asia/Bangkok");
//$tgl = date("d-m-Y H:i:s");
ob_clean();
$nama=$_GET['nama'];
if (isset($_GET['email']))
{
    $email = $_GET['email'];
    $result = mysqli_query($koneksi,"SELECT * FROM penilaian where karyawan = '$nama'");
	$dcek=mysqli_fetch_array($result);
	$dtgl=$dcek['tgl'];
	$karyawan=$dcek['karyawan'];
	$qdivisi=mysqli_query($koneksi,"select * from karyawan where nama = '$nama'");
	$ddivisi=mysqli_fetch_array($qdivisi);
	$divisi=$ddivisi['divisi'];
	$jabatan=$dcek['jabatan'];
	$penilai=$dcek['penilai'];
	$nilai1=$dcek['nilai1'];
	$nilai2=$dcek['nilai2'];
	$nilai3=$dcek['nilai3'];
	$nilai4=$dcek['nilai4'];
	$nilai5=$dcek['nilai5'];
	$nilai6=$dcek['nilai6'];
	$nilai7=$dcek['nilai7'];
	$nilai8=$dcek['nilai8'];
	$nilai9=$dcek['nilai9'];
	$nilai10=$dcek['nilai10'];
	$nilai11=$dcek['nilai11'];
	$nilai12=$dcek['nilai12'];
	$nilai13=$dcek['nilai13'];
	$nilai14=$dcek['nilai14'];
	$nilai15=$dcek['nilai15'];
	$nilai16=$dcek['nilai16'];
	$nilai17=$dcek['nilai17'];
	$nilai18=$dcek['nilai18'];
	$hasil1=$dcek['hasil1'];
	$hasil2=$dcek['hasil2'];
	$hasil3=$dcek['hasil3'];
	$hasil4=$dcek['hasil4'];
	$hasil5=$dcek['hasil5'];
	$hasil6=$dcek['hasil6'];
	$hasil7=$dcek['hasil7'];
	$hasil8=$dcek['hasil8'];
	$hasil9=$dcek['hasil9'];
	$hasil10=$dcek['hasil10'];
	$hasil11=$dcek['hasil11'];
	$hasil12=$dcek['hasil12'];
	$hasil13=$dcek['hasil13'];
	$hasil14=$dcek['hasil14'];
	$hasil15=$dcek['hasil15'];
	$hasil16=$dcek['hasil16'];
	$hasil17=$dcek['hasil17'];
	$hasil18=$dcek['hasil18'];
	$hasil18=$dcek['hasil19'];
	$total1=$nilai1+$nilai2+$nilai3+$nilai4+$nilai5+$nilai6+$nilai7+$nilai8+$nilai9+$nilai10+$nilai11+$nilai12+$nilai13+$nilai14+$nilai15+$nilai16+$nilai17+$nilai18+$nilai19;
	$total2=$hasil1+$hasil2+$hasil3+$hasil4+$hasil5+$hasil6+$hasil7+$hasil8+$hasil9+$hasil10+$hasil11+$hasil12+$hasil13+$hasil14+$hasil15+$hasil16+$hasil17+$nilai18+$nilai19;
	$rata1=round($total1/19,2);
	$rata2=round($total2/19,2);
	$catatan1=$dcek['catatan1'];
	$catatan2=$dcek['catatan2'];
	$catatan3=$dcek['catatan3'];
	$catatan4=$dcek['catatan4'];
	$tgl=date("d-M-Y", strtotime($dtgl));
	$pdf = new FPDF("P","cm","A4");
	$pdf->SetMargins(1,0.5,1);
	$pdf->AliasNbPages();
	$pdf->AddPage();
	$pdf->SetFont('Times','B',11);
	$pdf->MultiCell(19.5,0.5,'',0,'L'); 
	$pdf->SetX(4);   
	$pdf->SetFont('Arial','B',12);
	$pdf->SetX(1);
	$pdf->Cell(3.5,1.8, '', 1, 0, 'C');
	$pdf->Image('images/logo.PNG',1.1,1.1,3.3,1.5);
	$pdf->SetX(4.5);
	$pdf->Cell(7, 0.6, 'SISTEM MANAJEMEN MUTU', 'LTR', 0, 'C');
	$pdf->SetFont('Arial','',10);
	$pdf->Cell(8.3, 0.6, 'F-RC-SDM-10', 'LTR', 1, 'C');
	$pdf->SetX(4.5);
	$pdf->SetFont('Arial','B',12);
	$pdf->Cell(7, 0.6, 'PT RESIK CEMERLANG', 'LBR', 0, 'C');
	$pdf->SetFont('Arial','',10);
	$pdf->Cell(8.3, 0.6, 'FORM EVALUASI PENILAIAN KINERJA', 'LBR', 1, 'C');
	$pdf->SetX(4.5);
  	$pdf->Cell(4, 0.6, 'Halaman    : 1 dari 2', 'LB', 0, 'L');
	$pdf->Cell(3, 0.6, 'Revisi : 01', 'LBR', 0, 'C');
	$pdf->Cell(8.3, 0.6, 'Tanggal : '.$tgl, 'LBR', 1, 'C');
	$pdf->SetX(1);
	$pdf->SetFont('Arial','B',12);
	$pdf->Cell(18.8, 0.6, 'FORMULIR PENILAIAN KINERJA', 'LBR', 1, 'C');
	$pdf->SetFont('Arial','',10);
	$pdf->SetX(1);
	$pdf->Cell(18.8, 0.2, '', 'LR', 1, 'L');
	$pdf->SetX(1);
	$pdf->Cell(3, 0.6, 'Divisi', 'L', 0, 'L');
	$pdf->Cell(7, 0.6, ': '.$divisi, 0, 0, 'L');
	$pdf->SetX(11.5);
	$pdf->Cell(3, 0.6, 'Masa Kerja', 0, 0, 'L');
	$pdf->Cell(5.3, 0.6, ': ................', 'R', 1, 'L');
	$pdf->SetX(1);
	$pdf->Cell(3, 0.6, 'Nama', 'L', 0, 'L');
	$pdf->Cell(7, 0.6, ': '.$karyawan, 0, 0, 'L');
	$pdf->SetX(11.5);
	$pdf->Cell(3, 0.6, 'Gol/Jabatan', 0, 0, 'L');
	$pdf->Cell(5.3, 0.6, ': '.$jabatan, 'R', 1, 'L');
	$pdf->Cell(3, 0.6, 'NIK', 'L', 0, 'L');
	$pdf->Cell(7, 0.6, ': ................', 0, 0, 'L');
	$pdf->SetX(11.5);
	$pdf->Cell(3, 0.6, 'Semester', 0, 0, 'L');
	$pdf->Cell(5.3, 0.6, ': ................', 'R', 1, 'L');
	$pdf->Cell(18.8, 0.1, '', 'LR', 1, 'L');
	$pdf->Line(1,5.4,19.8,5.4);
	$pdf->SetLineWidth(0);      
	$pdf->Line(1,5.47,19.8,5.47);   
	$pdf->SetLineWidth(0);
	$pdf->SetFont('Arial','B',12);
	$pdf->Cell(1.2, 1.2, 'NO', 'LBR', 0, 'C');  
	$pdf->Cell(9.3, 1.2, 'ASPEK PENILAIAN', 'LBR', 0, 'C');
	$pdf->SetFont('Arial','',10);
	$pdf->Cell(8.3, 0.6, 'SKOR PENILAIAN', 'LBR', 1, 'C');
	$pdf->SetX(11.5);
	$pdf->Cell(4.15, 0.6, 'NILAI I', 'LBR', 0, 'C');
	$pdf->Cell(4.15, 0.6, 'NILAI II', 'LBR', 1, 'C');
	$pdf->Line(1,6.76,19.8,6.76);
	$pdf->SetLineWidth(0);      
	$pdf->SetX(1);
	$pdf->SetFont('Arial','B',10);
	$pdf->Cell(18.8, 0.6, 'A. ASPEK TEKNIS PEKERJAAN', 'LBR', 1, 'L');
	$pdf->SetFont('Arial','',10);
	$pdf->Cell(1.2, 0.53, '1', 'LBR', 0, 'C'); 
	$pdf->Cell(9.3, 0.53, 'Efektifitas & Efisiensi Kerja ', 'LBR', 0, 'L');
	$pdf->SetX(11.5);
	$pdf->Cell(4.15, 0.53, $nilai1, 'LBR', 0, 'C');
	$pdf->Cell(4.15, 0.53, $hasil1, 'LBR', 1, 'C');
	$pdf->Cell(1.2, 0.53, '2', 'LBR', 0, 'C'); 
	$pdf->Cell(9.3, 0.53, 'Ketepatan waktu dalam mengerjakan tugas (Reporting)', 'LBR', 0, 'L');
	$pdf->SetX(11.5);
	$pdf->Cell(4.15, 0.53, $nilai2, 'LBR', 0, 'C');
	$pdf->Cell(4.15, 0.53, $hasil2, 'LBR', 1, 'C');
	$pdf->Cell(1.2, 0.53, '3', 'LBR', 0, 'C'); 
	$pdf->Cell(9.3, 0.53, 'Memiliki Skill/kemampuan yang memadai pada bidangnya', 'LBR', 0, 'L');
	$pdf->SetX(11.5);
	$pdf->Cell(4.15, 0.53, $nilai3, 'LBR', 0, 'C');
	$pdf->Cell(4.15, 0.53, $hasil3, 'LBR', 1, 'C');
	$pdf->Cell(1.2, 0.53, '4', 'LBR', 0, 'C'); 
	$pdf->Cell(9.3, 0.53, 'Kemampuan mencapai target/ standar Perusahaan', 'LBR', 0, 'L');
	$pdf->SetX(11.5);
	$pdf->Cell(4.15, 0.53, $nilai4, 'LBR', 0, 'C');
	$pdf->Cell(4.15, 0.53, $hasil4, 'LBR', 1, 'C');
	$pdf->Cell(1.2, 0.53, '5', 'LBR', 0, 'C'); 
	$pdf->Cell(9.3, 0.53, 'Merealisasikan Pekerjaan Sesuai Job Desc', 'LBR', 0, 'L');
	$pdf->SetX(11.5);
	$pdf->Cell(4.15, 0.53, $nilai5, 'LBR', 0, 'C');
	$pdf->Cell(4.15, 0.53, $hasil5, 'LBR', 1, 'C');
	$pdf->Cell(1.2, 0.53, '6', 'LBR', 0, 'C'); 
	$pdf->Cell(9.3, 0.53, 'Responsif dalam komunikasi (Email, WhatsApp, Telepon)', 'LBR', 0, 'L');
	$pdf->SetX(11.5);
	$pdf->Cell(4.15, 0.53, $nilai6, 'LBR', 0, 'C');
	$pdf->Cell(4.15, 0.53, $hasil6, 'LBR', 1, 'C');
	$pdf->Cell(1.2, 0.53, '7', 'LBR', 0, 'C'); 
	$pdf->Cell(9.3, 0.53, 'Teliti dan cermat dalam menyelesaikan pekerjaan ', 'LBR', 0, 'L');
	$pdf->SetX(11.5);
	$pdf->Cell(4.15, 0.53, $nilai7, 'LBR', 0, 'C');
	$pdf->Cell(4.15, 0.53, $hasil7, 'LBR', 1, 'C');
	$pdf->Cell(1.2, 0.53, '8', 'LBR', 0, 'C'); 
	$pdf->Cell(9.3, 0.53, 'Tertib Administrasi', 'LBR', 0, 'L');
	$pdf->SetX(11.5);
	$pdf->Cell(4.15, 0.53, $nilai8, 'LBR', 0, 'C');
	$pdf->Cell(4.15, 0.53, $hasil8, 'LBR', 1, 'C');
	$pdf->Cell(10.5, 0.53, 'CATATAN I :', 'LR', 0, 'L');
	$pdf->Cell(8.3, 0.53, 'CATATAN II :', 'LR', 1, 'L');
	$pdf->Cell(10.5, 0.53, substr($catatan1,0,50), 'LR', 0, 'L');
	$pdf->Cell(8.3, 0.53, '', 'LR', 1, 'L');
	$pdf->Cell(10.5, 0.53, substr($catatan1,50,50), 'LR', 0, 'L');
	$pdf->Cell(8.3, 0.53, '', 'LR', 1, 'L');

	$pdf->SetFont('Arial','B',10);
	$pdf->Cell(18.8, 0.6, 'B. ASPEK NON TEKNIS', 1, 1, 'L');
	$pdf->SetFont('Arial','',10);
	$pdf->Cell(1.2, 0.535, '1', 'LBR', 0, 'C'); 
	$pdf->Cell(9.3, 0.53, 'Inisiatif', 'LBR', 0, 'L');
	$pdf->SetX(11.5);
	$pdf->Cell(4.15, 0.53, $nilai9, 'LBR', 0, 'C');
	$pdf->Cell(4.15, 0.53, $hasil9, 'LBR', 1, 'C');
	$pdf->Cell(1.2, 0.53, '2', 'LBR', 0, 'C'); 
	$pdf->Cell(9.3, 0.53, 'Integritas', 'LBR', 0, 'L');
	$pdf->SetX(11.5);
	$pdf->Cell(4.15, 0.53, $nilai10, 'LBR', 0, 'C');
	$pdf->Cell(4.15, 0.53, $hasil10, 'LBR', 1, 'C');
	$pdf->Cell(1.2, 0.53, '3', 'LBR', 0, 'C'); 
	$pdf->Cell(9.3, 0.53, 'Kerjasama & Koordinasi antar bagian', 'LBR', 0, 'L');
	$pdf->SetX(11.5);
	$pdf->Cell(4.15, 0.53, $nilai11, 'LBR', 0, 'C');
	$pdf->Cell(4.15, 0.53, $hasil11, 'LBR', 1, 'C');
	$pdf->Cell(1.2, 0.53, '4', 'LBR', 0, 'C'); 
	$pdf->Cell(9.3, 0.53, 'Tanggung Jawab & Loyalitas', 'LBR', 0, 'L');
	$pdf->SetX(11.5);
	$pdf->Cell(4.15, 0.53, $nilai12, 'LBR', 0, 'C');
	$pdf->Cell(4.15, 0.53, $hasil12, 'LBR', 1, 'C');
	$pdf->Cell(1.2, 0.53, '5', 'LBR', 0, 'C'); 
	$pdf->Cell(9.3, 0.53, 'Perilaku dan Sikap', 'LBR', 0, 'L');
	$pdf->SetX(11.5);
	$pdf->Cell(4.15, 0.53, $nilai13, 'LBR', 0, 'C');
    $pdf->Cell(4.15, 0.53, $hasil13, 'LBR', 1, 'C');
	$pdf->Cell(1.2, 0.53, '6', 'LBR', 0, 'C'); 
	$pdf->Cell(9.3, 0.53, 'Akuntabilitas', 'LBR', 0, 'L');
	$pdf->SetX(11.5);
	$pdf->Cell(4.15, 0.53, $nilai14, 'LBR', 0, 'C');
	$pdf->Cell(4.15, 0.53, $hasil14, 'LBR', 1, 'C');
	$pdf->Cell(1.2, 0.53, '7', 'LBR', 0, 'C'); 
	$pdf->Cell(9.3, 0.53, 'Kedisiplinan', 'LBR', 0, 'L');
	$pdf->SetX(11.5);
	$pdf->Cell(4.15, 0.53, $nilai15, 'LBR', 0, 'C');
	$pdf->Cell(4.15, 0.53, $hasil15, 'LBR', 1, 'C');
	$pdf->Cell(10.5, 0.53, 'CATATAN I :', 'LR', 0, 'L');
	$pdf->Cell(8.3, 0.53, 'CATATAN II :', 'LR', 1, 'L');
	$pdf->Cell(10.5, 0.53, substr($catatan2,0,50), 'LR', 0, 'L');
	$pdf->Cell(8.3, 0.53, '', 'LR', 1, 'L');
	$pdf->Cell(10.5, 0.53, substr($catatan2,50,50), 'LR', 0, 'L');
	$pdf->Cell(8.3, 0.53, '', 'LR', 1, 'L');
	$pdf->SetFont('Arial','B',10);

	$pdf->Cell(18.8, 0.6, 'C. ASPEK KEPEMIMPINAN', 1, 1, 'L');
	$pdf->SetFont('Arial','',10);
	$pdf->Cell(1.2, 0.53, '1', 'LBR', 0, 'C'); 
	$pdf->Cell(9.3, 0.53, ' Koordinasi anggota dan Kontrol anggota ', 'LBR', 0, 'L');
	$pdf->SetX(11.5);
	$pdf->Cell(4.15, 0.53, $nilai16, 'LBR', 0, 'C');
	$pdf->Cell(4.15, 0.53, $hasil16, 'LBR', 1, 'C');
	$pdf->Cell(1.2, 0.53, '2', 'LBR', 0, 'C'); 
	$pdf->Cell(9.3, 0.53, 'Ketepatan dan Kecepatan dalam Pengambilan Keputusan', 'LBR', 0, 'L');
	$pdf->SetX(11.5);
	$pdf->Cell(4.15, 0.53, $nilai17, 'LBR', 0, 'C');
	$pdf->Cell(4.15, 0.53, $hasil17, 'LBR', 1, 'C');
	$pdf->Cell(1.2, 0.53, '3', 'LBR', 0, 'C'); 
	$pdf->Cell(9.3, 0.53, 'Evaluasi dan pembinaan anggota', 'LBR', 0, 'L');
	$pdf->SetX(11.5);
	$pdf->Cell(4.15, 0.53, $nilai18, 'LBR', 0, 'C');
	$pdf->Cell(4.15, 0.53, $hasil18, 'LBR', 1, 'C');
	$pdf->Cell(1.2, 0.53, '4', 'LBR', 0, 'C'); 
	$pdf->Cell(9.3, 0.53, 'Delegasi tanggung jawab dan wewenang', 'LBR', 0, 'L');
	$pdf->SetX(11.5);
	$pdf->Cell(4.15, 0.53, $nilai19, 'LBR', 0, 'C');
	$pdf->Cell(4.15, 0.53, $hasil19, 'LBR', 1, 'C');
	$pdf->Cell(10.5, 0.53, 'CATATAN I :', 'LR', 0, 'L');
	$pdf->Cell(8.3, 0.53, 'CATATAN II :', 'LR', 1, 'L');
    $pdf->Cell(10.5, 0.53, substr($catatan3,0,50), 'LR', 0, 'L');
	$pdf->Cell(8.3, 0.53, '', 'LR', 1, 'L');
	$pdf->Cell(10.5, 0.53, substr($catatan3,50,50), 'LR', 0, 'L');
	$pdf->Cell(8.3, 0.53, '', 'LR', 1, 'L');


	$pdf->Cell(10.5, 0.53, 'Total Nilai', 1, 0, 'C');
	$pdf->Cell(4.15, 0.53, $total1, 1, 0, 'C');
	$pdf->Cell(4.15, 0.53, $total2, 1, 1, 'C');
	$pdf->Cell(10.5, 0.53, 'Rata - rata Nilai', 1, 0, 'C');
	$pdf->Cell(4.15, 0.53, $rata1, 1, 0, 'C');
	$pdf->Cell(4.15, 0.53, $rata2, 1, 1, 'C');
	$pdf->Cell(18.8, 0.53, 'CATATAN (Direktur Utama/Wakil Direktur Utama) :', 'LR', 0, 'L');
	$pdf->Cell(18.8, 0.53, '', 'LR', 1, 'L');
	$pdf->Cell(18.8, 0.53, '', 'LR', 1, 'L');
	$pdf->Cell(18.8, 0.53, '', 'LR', 1, 'L');
	$pdf->Cell(4.7, 0.53, 'Dibuat Oleh', 1, 0, 'C');
	$pdf->Cell(9.4, 0.53, 'Diketahui Oleh', 1, 0, 'C');
	$pdf->Cell(4.7, 0.53, 'Disetujui Oleh', 1, 1, 'C');
	$pdf->Cell(4.7, 1.5, '', 'LR', 0, 'L');
	$pdf->Cell(4.7, 1.5, '', 'LR', 0, 'L');
	$pdf->Cell(4.7, 1.5, '', 'LR', 0, 'L');
	$pdf->Cell(4.7, 1.5, '', 'LR', 1, 'L');
	$pdf->Cell(4.7, 0.53, $penilai, 1, 0, 'C');
	$pdf->Cell(4.7, 0.53, 'Direktur Divisi', 1, 0, 'C');
	$pdf->Cell(4.7, 0.53, 'Direktur SDM', 1, 0, 'C');
	$pdf->Cell(4.7, 0.53, 'Direktur Utama / Wakil', 1, 0, 'C');
	$pdf->Output("form_pdf/Form Penilaian ".$karyawan.".pdf","F");
	$fpdf="Form Penilaian ".$karyawan.".pdf";
	?>
	<script>
	  var email = "<?php echo $email; ?>";
	  var nama = "<?php echo $karyawan; ?>";
	  var pdf = "<?php echo $fpdf; ?>";
	  window.location.href = 'kirim_email_approval.php?nama='+nama+'&email='+email+'&pdf='+pdf;
	</script>
	<?php
}
else
{
 	$result = mysqli_query($koneksi,"SELECT * FROM penilaian where id = '$id'");
	$dcek=mysqli_fetch_array($result);
	$dtgl=$dcek['tgl'];
	$karyawan=$dcek['karyawan'];
	$qdivisi=mysqli_query($koneksi,"select * from karyawan where nama = '$karyawan'");
	$ddivisi=mysqli_fetch_array($qdivisi);
	$divisi=$ddivisi['divisi'];
	$jabatan=$dcek['jabatan'];
	$penilai=$dcek['penilai'];
	$nilai1=$dcek['nilai1'];
	$nilai2=$dcek['nilai2'];
	$nilai3=$dcek['nilai3'];
	$nilai4=$dcek['nilai4'];
	$nilai5=$dcek['nilai5'];
	$nilai6=$dcek['nilai6'];
	$nilai7=$dcek['nilai7'];
	$nilai8=$dcek['nilai8'];
	$nilai9=$dcek['nilai9'];
	$nilai10=$dcek['nilai10'];
	$nilai11=$dcek['nilai11'];
	$nilai12=$dcek['nilai12'];
	$nilai13=$dcek['nilai13'];
	$nilai14=$dcek['nilai14'];
	$nilai15=$dcek['nilai15'];
	$nilai16=$dcek['nilai16'];
	$nilai17=$dcek['nilai17'];
	$hasil1=$dcek['hasil1'];
	$hasil2=$dcek['hasil2'];
	$hasil3=$dcek['hasil3'];
	$hasil4=$dcek['hasil4'];
	$hasil5=$dcek['hasil5'];
	$hasil6=$dcek['hasil6'];
	$hasil7=$dcek['hasil7'];
	$hasil8=$dcek['hasil8'];
	$hasil9=$dcek['hasil9'];
	$hasil10=$dcek['hasil10'];
	$hasil11=$dcek['hasil11'];
	$hasil12=$dcek['hasil12'];
	$hasil13=$dcek['hasil13'];
	$hasil14=$dcek['hasil14'];
	$hasil15=$dcek['hasil15'];
	$hasil16=$dcek['hasil16'];
	$hasil17=$dcek['hasil17'];
	$hasil18=$dcek['hasil18'];
	$hasil18=$dcek['hasil19'];
	$total1=$nilai1+$nilai2+$nilai3+$nilai4+$nilai5+$nilai6+$nilai7+$nilai8+$nilai9+$nilai10+$nilai11+$nilai12+$nilai13+$nilai14+$nilai15+$nilai16+$nilai17+$nilai18+$nilai19;
	$total2=$hasil1+$hasil2+$hasil3+$hasil4+$hasil5+$hasil6+$hasil7+$hasil8+$hasil9+$hasil10+$hasil11+$hasil12+$hasil13+$hasil14+$hasil15+$hasil16+$hasil17+$nilai18+$nilai19;
	$rata1=round($total1/19,2);
	$rata2=round($total2/19,2);
	$catatan1=$dcek['catatan1'];
	$catatan2=$dcek['catatan2'];
	$catatan3=$dcek['catatan3'];
	$catatan4=$dcek['catatan4'];
	$tgl=date("d-M-Y", strtotime($dtgl));
	$pdf = new FPDF("P","cm","A4");
	$pdf->SetMargins(1,0.5,1);
	$pdf->AliasNbPages();
	$pdf->AddPage();
	$pdf->SetFont('Times','B',11);
	$pdf->MultiCell(19.5,0.5,'',0,'L'); 
	$pdf->SetX(4);   
	$pdf->SetFont('Arial','B',12);
	$pdf->SetX(1);
	$pdf->Cell(3.5,1.8, '', 1, 0, 'C');
	$pdf->Image('images/logo.JPG',1.1,1.1,3.3,1.5);
	$pdf->SetX(4.5);
	$pdf->Cell(7, 0.6, 'SISTEM MANAJEMEN MUTU', 'LTR', 0, 'C');
	$pdf->SetFont('Arial','',10);
	$pdf->Cell(8.3, 0.6, 'F-RC-SDM-09', 'LTR', 1, 'C');
	$pdf->SetX(4.5);
	$pdf->SetFont('Arial','B',12);
	$pdf->Cell(7, 0.6, 'PT RESIK CEMERLANG', 'LBR', 0, 'C');
	$pdf->SetFont('Arial','',10);
	$pdf->Cell(8.3, 0.6, 'FORM EVALUASI PENILAIAN KINERJA', 'LBR', 1, 'C');
	$pdf->SetX(4.5);
  	$pdf->Cell(4, 0.6, 'Halaman    : 1 dari 1', 'LB', 0, 'L');
	$pdf->Cell(3, 0.6, 'Revisi : 00', 'LBR', 0, 'C');
	$pdf->Cell(8.3, 0.6, 'Tanggal : '.$tgl, 'LBR', 1, 'C');
	$pdf->SetX(1);
	$pdf->SetFont('Arial','B',12);
	$pdf->Cell(18.8, 0.6, 'FORMULIR PENILAIAN KINERJA', 'LBR', 1, 'C');
	$pdf->SetFont('Arial','',10);
	$pdf->SetX(1);
	$pdf->Cell(18.8, 0.2, '', 'LR', 1, 'L');
	$pdf->SetX(1);
	$pdf->Cell(3, 0.6, 'Divisi', 'L', 0, 'L');
	$pdf->Cell(7, 0.6, ': '.$divisi, 0, 0, 'L');
	$pdf->SetX(11.5);
	$pdf->Cell(3, 0.6, 'Masa Kerja', 0, 0, 'L');
	$pdf->Cell(5.3, 0.6, ': ................', 'R', 1, 'L');
	$pdf->SetX(1);
	$pdf->Cell(3, 0.6, 'Nama', 'L', 0, 'L');
	$pdf->Cell(7, 0.6, ': '.$karyawan, 0, 0, 'L');
	$pdf->SetX(11.5);
	$pdf->Cell(3, 0.6, 'Gol/Jabatan', 0, 0, 'L');
	$pdf->Cell(5.3, 0.6, ': '.$jabatan, 'R', 1, 'L');
	$pdf->Cell(3, 0.6, 'NIK', 'L', 0, 'L');
	$pdf->Cell(7, 0.6, ': ................', 0, 0, 'L');
	$pdf->SetX(11.5);
	$pdf->Cell(3, 0.6, 'Semester', 0, 0, 'L');
	$pdf->Cell(5.3, 0.6, ': ................', 'R', 1, 'L');
	$pdf->Cell(18.8, 0.1, '', 'LR', 1, 'L');
	$pdf->Line(1,5.4,19.8,5.4);
	$pdf->SetLineWidth(0);      
	$pdf->Line(1,5.47,19.8,5.47);   
	$pdf->SetLineWidth(0);
	$pdf->SetFont('Arial','B',12);
	$pdf->Cell(1.2, 1.2, 'NO', 'LBR', 0, 'C');  
	$pdf->Cell(9.3, 1.2, 'ASPEK PENILAIAN', 'LBR', 0, 'C');
	$pdf->SetFont('Arial','',10);
	$pdf->Cell(8.3, 0.6, 'SKOR PENILAIAN', 'LBR', 1, 'C');
	$pdf->SetX(11.5);
	$pdf->Cell(4.15, 0.6, 'NILAI I', 'LBR', 0, 'C');
	$pdf->Cell(4.15, 0.6, 'NILAI II', 'LBR', 1, 'C');
	$pdf->Line(1,6.76,19.8,6.76);
	$pdf->SetLineWidth(0);      
	$pdf->SetX(1);
	$pdf->SetFont('Arial','B',10);
	$pdf->Cell(18.8, 0.6, 'A. ASPEK TEKNIS PEKERJAAN', 'LBR', 1, 'L');
	$pdf->SetFont('Arial','',10);
	$pdf->Cell(1.2, 0.53, '1', 'LBR', 0, 'C'); 
	$pdf->Cell(9.3, 0.53, 'Efektifitas & Efisiensi Kerja ', 'LBR', 0, 'L');
	$pdf->SetX(11.5);
	$pdf->Cell(4.15, 0.53, $nilai1, 'LBR', 0, 'C');
	$pdf->Cell(4.15, 0.53, $hasil1, 'LBR', 1, 'C');
	$pdf->Cell(1.2, 0.53, '2', 'LBR', 0, 'C'); 
	$pdf->Cell(9.3, 0.53, 'Ketepatan waktu dalam mengerjakan tugas (Reporting)', 'LBR', 0, 'L');
	$pdf->SetX(11.5);
	$pdf->Cell(4.15, 0.53, $nilai2, 'LBR', 0, 'C');
	$pdf->Cell(4.15, 0.53, $hasil2, 'LBR', 1, 'C');
	$pdf->Cell(1.2, 0.53, '3', 'LBR', 0, 'C'); 
	$pdf->Cell(9.3, 0.53, 'Memiliki Skill/kemampuan yang memadai pada bidangnya', 'LBR', 0, 'L');
	$pdf->SetX(11.5);
	$pdf->Cell(4.15, 0.53, $nilai3, 'LBR', 0, 'C');
	$pdf->Cell(4.15, 0.53, $hasil3, 'LBR', 1, 'C');
	$pdf->Cell(1.2, 0.53, '4', 'LBR', 0, 'C'); 
	$pdf->Cell(9.3, 0.53, 'Kemampuan mencapai target/ standar Perusahaan', 'LBR', 0, 'L');
	$pdf->SetX(11.5);
	$pdf->Cell(4.15, 0.53, $nilai4, 'LBR', 0, 'C');
	$pdf->Cell(4.15, 0.53, $hasil4, 'LBR', 1, 'C');
	$pdf->Cell(1.2, 0.53, '5', 'LBR', 0, 'C'); 
	$pdf->Cell(9.3, 0.53, 'Merealisasikan Pekerjaan Sesuai Job Desc', 'LBR', 0, 'L');
	$pdf->SetX(11.5);
	$pdf->Cell(4.15, 0.53, $nilai5, 'LBR', 0, 'C');
	$pdf->Cell(4.15, 0.53, $hasil5, 'LBR', 1, 'C');
	$pdf->Cell(1.2, 0.53, '6', 'LBR', 0, 'C'); 
	$pdf->Cell(9.3, 0.53, 'Responsif dalam komunikasi (Email, WhatsApp, Telepon)', 'LBR', 0, 'L');
	$pdf->SetX(11.5);
	$pdf->Cell(4.15, 0.53, $nilai6, 'LBR', 0, 'C');
	$pdf->Cell(4.15, 0.53, $hasil6, 'LBR', 1, 'C');
	$pdf->Cell(1.2, 0.53, '7', 'LBR', 0, 'C'); 
	$pdf->Cell(9.3, 0.53, 'Teliti dan cermat dalam menyelesaikan pekerjaan ', 'LBR', 0, 'L');
	$pdf->SetX(11.5);
	$pdf->Cell(4.15, 0.53, $nilai7, 'LBR', 0, 'C');
	$pdf->Cell(4.15, 0.53, $hasil7, 'LBR', 1, 'C');
	$pdf->Cell(10.5, 0.53, 'CATATAN I :', 'LR', 0, 'L');
	$pdf->Cell(8.3, 0.53, 'CATATAN II :', 'LR', 1, 'L');
	$pdf->Cell(10.5, 0.53, substr($catatan1,0,50), 'LR', 0, 'L');
	$pdf->Cell(8.3, 0.53, '', 'LR', 1, 'L');
	$pdf->Cell(10.5, 0.53, substr($catatan1,50,50), 'LR', 0, 'L');
	$pdf->Cell(8.3, 0.53, '', 'LR', 1, 'L');

	$pdf->SetFont('Arial','B',10);
	$pdf->Cell(18.8, 0.6, 'B. ASPEK NON TEKNIS', 1, 1, 'L');
	$pdf->SetFont('Arial','',10);
	$pdf->Cell(1.2, 0.53, '1', 'LBR', 0, 'C'); 
	$pdf->Cell(9.3, 0.53, 'Tertib Administrasi', 'LBR', 0, 'L');
	$pdf->SetX(11.5);
	$pdf->Cell(4.15, 0.53, $nilai8, 'LBR', 0, 'C');
	$pdf->Cell(4.15, 0.53, $hasil8, 'LBR', 1, 'C');
	$pdf->Cell(1.2, 0.535, '2', 'LBR', 0, 'C'); 
	$pdf->Cell(9.3, 0.53, 'Inisiatif', 'LBR', 0, 'L');
	$pdf->SetX(11.5);
	$pdf->Cell(4.15, 0.53, $nilai9, 'LBR', 0, 'C');
	$pdf->Cell(4.15, 0.53, $hasil9, 'LBR', 1, 'C');
	$pdf->Cell(1.2, 0.53, '3', 'LBR', 0, 'C'); 
	$pdf->Cell(9.3, 0.53, 'Integritas dan Perilaku Baik (Attitude) ', 'LBR', 0, 'L');
	$pdf->SetX(11.5);
	$pdf->Cell(4.15, 0.53, $nilai10, 'LBR', 0, 'C');
	$pdf->Cell(4.15, 0.53, $hasil10, 'LBR', 1, 'C');
	$pdf->Cell(1.2, 0.53, '4', 'LBR', 0, 'C'); 
	$pdf->Cell(9.3, 0.53, 'Kerjasama & Koordinasi antar bagian', 'LBR', 0, 'L');
	$pdf->SetX(11.5);
	$pdf->Cell(4.15, 0.53, $nilai11, 'LBR', 0, 'C');
	$pdf->Cell(4.15, 0.53, $hasil11, 'LBR', 1, 'C');
	$pdf->Cell(1.2, 0.53, '5', 'LBR', 0, 'C'); 
	$pdf->Cell(9.3, 0.53, 'Tanggung Jawab & Loyalitas', 'LBR', 0, 'L');
	$pdf->SetX(11.5);
	$pdf->Cell(4.15, 0.53, $nilai12, 'LBR', 0, 'C');
	$pdf->Cell(4.15, 0.53, $hasil12, 'LBR', 1, 'C');
	$pdf->Cell(10.5, 0.53, 'CATATAN I :', 'LR', 0, 'L');
	$pdf->Cell(8.3, 0.53, 'CATATAN II :', 'LR', 1, 'L');
	$pdf->Cell(10.5, 0.53, substr($catatan2,0,50), 'LR', 0, 'L');
	$pdf->Cell(8.3, 0.53, '', 'LR', 1, 'L');
	$pdf->Cell(10.5, 0.53, substr($catatan2,50,50), 'LR', 0, 'L');
	$pdf->Cell(8.3, 0.53, '', 'LR', 1, 'L');
	$pdf->SetFont('Arial','B',10);

	$pdf->Cell(18.8, 0.6, 'C. ASPEK KEPEMIMPINAN', 1, 1, 'L');
	$pdf->SetFont('Arial','',10);
	$pdf->Cell(1.2, 0.53, '1', 'LBR', 0, 'C'); 
	$pdf->Cell(9.3, 0.53, 'Koordinasi anggota dan Kontrol anggota', 'LBR', 0, 'L');
	$pdf->SetX(11.5);
	$pdf->Cell(4.15, 0.53, $nilai13, 'LBR', 0, 'C');
    $pdf->Cell(4.15, 0.53, $hasil13, 'LBR', 1, 'C');
	$pdf->Cell(1.2, 0.53, '2', 'LBR', 0, 'C'); 
	$pdf->Cell(9.3, 0.53, 'Intregitas', 'LBR', 0, 'L');
	$pdf->SetX(11.5);
	$pdf->Cell(4.15, 0.53, $nilai14, 'LBR', 0, 'C');
	$pdf->Cell(4.15, 0.53, $hasil14, 'LBR', 1, 'C');
	$pdf->Cell(1.2, 0.53, '3', 'LBR', 0, 'C'); 
	$pdf->Cell(9.3, 0.53, 'Evaluasi dan pembinaan anggota', 'LBR', 0, 'L');
	$pdf->SetX(11.5);
	$pdf->Cell(4.15, 0.53, $nilai15, 'LBR', 0, 'C');
	$pdf->Cell(4.15, 0.53, $hasil15, 'LBR', 1, 'C');
	$pdf->Cell(1.2, 0.53, '4', 'LBR', 0, 'C'); 
	$pdf->Cell(9.3, 0.53, 'Delegasi tanggung jawab dan wewenang', 'LBR', 0, 'L');
	$pdf->SetX(11.5);
	$pdf->Cell(4.15, 0.53, $nilai16, 'LBR', 0, 'C');
	$pdf->Cell(4.15, 0.53, $hasil16, 'LBR', 1, 'C');
	$pdf->Cell(1.2, 0.53, '5', 'LBR', 0, 'C'); 
	$pdf->Cell(9.3, 0.53, 'Akuntabilitas', 'LBR', 0, 'L');
	$pdf->SetX(11.5);
	$pdf->Cell(4.15, 0.53, $nilai17, 'LBR', 0, 'C');
	$pdf->Cell(4.15, 0.53, $hasil17, 'LBR', 1, 'C');
	$pdf->Cell(10.5, 0.53, 'CATATAN I :', 'LR', 0, 'L');
	$pdf->Cell(8.3, 0.53, 'CATATAN II :', 'LR', 1, 'L');
    $pdf->Cell(10.5, 0.53, substr($catatan3,0,50), 'LR', 0, 'L');
	$pdf->Cell(8.3, 0.53, '', 'LR', 1, 'L');
	$pdf->Cell(10.5, 0.53, substr($catatan3,50,50), 'LR', 0, 'L');
	$pdf->Cell(8.3, 0.53, '', 'LR', 1, 'L');

	$pdf->Cell(18.8, 0.6, 'C. ASPEK KEPRIBADIAN', 1, 1, 'L');
	$pdf->SetFont('Arial','',10);
	$pdf->Cell(1.2, 0.53, '1', 'LBR', 0, 'C'); 
	$pdf->Cell(9.3, 0.53, 'Perilaku, Sikap dan Penampilan.', 'LBR', 0, 'L');
	$pdf->SetX(11.5);
	$pdf->Cell(4.15, 0.53, $nilai18, 'LBR', 0, 'C');
    $pdf->Cell(4.15, 0.53, $hasil18, 'LBR', 1, 'C');
	$pdf->Cell(1.2, 0.53, '2', 'LBR', 0, 'C'); 
	$pdf->Cell(9.3, 0.53, 'Kerapihan dan Penampilan', 'LBR', 0, 'L');
	$pdf->SetX(11.5);
	$pdf->Cell(4.15, 0.53, $nilai19, 'LBR', 0, 'C');
	$pdf->Cell(4.15, 0.53, $hasil19, 'LBR', 1, 'C');
	$pdf->Cell(10.5, 0.53, 'CATATAN I :', 'LR', 0, 'L');
	$pdf->Cell(8.3, 0.53, 'CATATAN II :', 'LR', 1, 'L');
    $pdf->Cell(10.5, 0.53, substr($catatan4,0,50), 'LR', 0, 'L');
	$pdf->Cell(8.3, 0.53, '', 'LR', 1, 'L');
	$pdf->Cell(10.5, 0.53, substr($catatan4,50,50), 'LR', 0, 'L');
	$pdf->Cell(8.3, 0.53, '', 'LR', 1, 'L');
	$pdf->Cell(10.5, 0.53, 'Total Nilai', 1, 0, 'C');
	$pdf->Cell(4.15, 0.53, $total1, 1, 0, 'C');
	$pdf->Cell(4.15, 0.53, $total2, 1, 1, 'C');
	$pdf->Cell(10.5, 0.53, 'Rata - rata Nilai', 1, 0, 'C');
	$pdf->Cell(4.15, 0.53, $rata1, 1, 0, 'C');
	$pdf->Cell(4.15, 0.53, $rata2, 1, 1, 'C');
	$pdf->Cell(18.8, 0.53, 'CATATAN (Direktur Utama/Wakil Direktur Utama) :', 'LR', 0, 'L');
	$pdf->Cell(18.8, 0.53, '', 'LR', 1, 'L');
	$pdf->Cell(18.8, 0.53, '', 'LR', 1, 'L');
	$pdf->Cell(18.8, 0.53, '', 'LR', 1, 'L');
	$pdf->Cell(4.7, 0.53, 'Dibuat Oleh', 1, 0, 'C');
	$pdf->Cell(9.4, 0.53, 'Diketahui Oleh', 1, 0, 'C');
	$pdf->Cell(4.7, 0.53, 'Disetujui Oleh', 1, 1, 'C');
	$pdf->Cell(4.7, 1.5, '', 'LR', 0, 'L');
	$pdf->Cell(4.7, 1.5, '', 'LR', 0, 'L');
	$pdf->Cell(4.7, 1.5, '', 'LR', 0, 'L');
	$pdf->Cell(4.7, 1.5, '', 'LR', 1, 'L');
	$pdf->Cell(4.7, 0.53, $penilai, 1, 0, 'C');
	$pdf->Cell(4.7, 0.53, 'Direktur Divisi', 1, 0, 'C');
	$pdf->Cell(4.7, 0.53, 'Direktur SDM', 1, 0, 'C');
	$pdf->Cell(4.7, 0.53, 'Direktur Utama / Wakil', 1, 0, 'C');
	$pdf->Output("form_pdf/Form Penilaian ".$karyawan.".pdf","F");
	$pdf='form_pdf/Form Penilaian '.$karyawan.'.pdf';
	$fpdf="Form Penilaian ".$karyawan.".pdf";
	?>
	<script>
	  var email = "<?php echo $email; ?>";
	  var nama = "<?php echo $karyawan; ?>";
	  var pdf = "<?php echo $fpdf; ?>";
	  window.location.href = 'kirim_email_approval.php?nama='+nama+'&email='+email+'&pdf='+pdf;
	</script>
	<?php
}	
?>

